...
local function tpr_accept(name, param)
	...
	if tpr_list[name] then
		...
		minetest.log("action", name2 .. " teleported to " .. name .. " via tpr.")
		tpr_list[name] = nil
	elseif tphr_list[name] then
		...
		minetest.log("action", name .. " teleported to " .. name2 .. " via tphr.")
		tphr_list[name] = nil
	else
		return
	end
...
--Disabled until it can detect if PVP is enabled.
--[[
-- Evade
